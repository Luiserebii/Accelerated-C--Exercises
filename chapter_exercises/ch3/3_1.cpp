...
I'm not sure I understand this question... doesn't this answer itself? If we do not know how many values remain to be read, assuming that the values are sorted, we can't determine where the "middle" unless we gather them up. As the hint says, we could have discarded the median, and thus would not be able to find the true one. Is there some formal proof solution I'm missing here?
