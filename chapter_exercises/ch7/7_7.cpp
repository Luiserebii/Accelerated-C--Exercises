...
int main() {
    ...
    for(map<string, vector<int> >::const_iterator i = ret.begin(); i != ret.end(); ++i) {
        if(i->second.size() == 1) {
            ...
            cout << i->first << " occurs on line: " << *(i->second.begin()) << endl;
            ...
            throw logic_error("Error: cross-referenced mapping contained word with an empty vector<int> (no line numbers found). This should be unreachable; this was found for the string: " + i->first);
        }
    }    
    return 0;
}
